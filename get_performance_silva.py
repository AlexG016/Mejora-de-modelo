...
from sklearn.metrics import matthews_corrcoef as mcc, confusion_matrix, \
    precision_score, recall_score, f1_score, accuracy_score, roc_auc_score, plot_confusion_matrix
...
for file in files:
    ...
    #%
    l, med, h = bootstrap_metrics(y_true, y_pred, [mcc, precision_score, recall_score, f1_score])
    
    for i,metric in zip(np.arange(4), ['mcc', 'p+', 'sensitivity', 'F-score']):
        print(metric)
        print(l[i], med[i], h[i])
